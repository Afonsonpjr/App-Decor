// App-Decor - Versao com carregamento flexivel de imagens (JPG, PNG, WEBP, etc)
// Branch: fix/mobile-input-surface-targets
...
function setupTeleport() {
    ...
    renderer.domElement.addEventListener('mousemove', (e) => {
        if (e.buttons === 2 && mouseDownTime > 0) {
    ...
    renderer.domElement.addEventListener('mouseup', (e) => {
        if (e.button === 2 && mouseDownTime > 0) {
            ...
            if (holdTime < 300 && !isMouseDragging && !controls.enabled) {
                performTeleport();
            }
...
function performTeleport() {
    ...
    if (floorIntersects.length > 0) {
        ...
        if (distance > 8) {
            showNotification('Muito longe!', 'error');
            return;
        }
...
// Materiais separados para parede, painel e piso demonstrativo
...
const demoFloorMaterial = new THREE.MeshStandardMaterial({ 
    color: 0x3a3a3a, 
    roughness: 0.5,
    metalness: 0.2
});

// Carrega texturas com fallback para multiplas extensoes
loadTextureFlexible('assets/textures/minerio', (texture) => {
    if (texture) {
        texture.repeat.set(4, 4);
        demoFloorMaterial.map = texture;
        demoFloorMaterial.needsUpdate = true;
    }
});
...
function createHUD() {
    ...
    instructions.innerHTML = `
        <strong>Decor Colors - Showroom</strong><br>
        PC: WASD/Setas - Mover | Mouse - Olhar<br>
        Clique direito curto - Teleporte<br>
        Clique direito longo - Capturar mouse<br>
        ESC - Sair do mouse<br>
        Mobile: Toque esquerdo - Mover | Deslizar - Olhar<br>
        <br>
        <strong>ACESSO PELO CELULAR:</strong><br>
        Use o link HTTPS do Cloudflare<br>
        <br>
...
setTimeout(() => {
    showNotification('Bem-vindo ao Showroom!', 'success');
}, 500);
