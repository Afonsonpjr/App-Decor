...
function resetAllInput() {
    ...
    if (virtualJoystick) { virtualJoystick.active = false; virtualJoystick.startX = 0; virtualJoystick.startY = 0; virtualJoystick.currentX = 0; virtualJoystick.currentY = 0; virtualJoystick.angle = 0; virtualJoystick.distance = 0; updateVirtualJoystickVisual(); }
    isTeleporting = false;
}
...
function updateMovement() {
    if (!moveState.isMoving) { moveState.velocity *= moveState.friction; if (Math.abs(moveState.velocity) < 0.001) moveState.velocity = 0; }
    else { if (moveState.velocity < moveState.maxVelocity) moveState.velocity += moveState.acceleration; }
    if (moveState.velocity > 0) {
        ...
        if (moveState.forward) moveDir.add(direction); if (moveState.backward) moveDir.sub(direction);
        if (moveState.left) moveDir.add(right); if (moveState.right) moveDir.sub(right);
        if (moveDir.length() > 0) { moveDir.normalize(); camera.position.addScaledVector(moveDir, moveState.velocity); }
    }
}
...
function createVirtualJoystick() {
    const joystickZone = document.createElement('div'); joystickZone.id = 'joystick-zone';
    ...
    const joystickKnob = document.createElement('div'); joystickKnob.id = 'joystick-knob';
    ...
    joystickZone.appendChild(joystickKnob); document.body.appendChild(joystickZone);
    document.addEventListener('touchstart', (e) => {
        const touch = e.touches[0];
        if (touch.clientX < window.innerWidth / 2 && touch.clientY > window.innerHeight / 2) {
            virtualJoystick.active = true; virtualJoystick.startX = touch.clientX; virtualJoystick.startY = touch.clientY;
            virtualJoystick.currentX = touch.clientX; virtualJoystick.currentY = touch.clientY;
            joystickZone.style.display = 'block'; joystickZone.style.left = (touch.clientX - 80) + 'px'; joystickZone.style.top = (touch.clientY - 80) + 'px';
        }
    });
    document.addEventListener('touchmove', (e) => {
        if (virtualJoystick.active) {
            const touch = e.touches[0]; virtualJoystick.currentX = touch.clientX; virtualJoystick.currentY = touch.clientY;
            const dx = touch.clientX - virtualJoystick.startX; const dy = touch.clientY - virtualJoystick.startY;
            const distance = Math.sqrt(dx * dx + dy * dy); const angle = Math.atan2(dy, dx);
            virtualJoystick.distance = Math.min(distance, virtualJoystick.maxDistance); virtualJoystick.angle = angle;
            const knobX = Math.cos(angle) * virtualJoystick.distance; const knobY = Math.sin(angle) * virtualJoystick.distance;
    ...
    document.addEventListener('touchend', () => {
        if (virtualJoystick.active) {
            virtualJoystick.active = false; virtualJoystick.distance = 0; joystickZone.style.display = 'none';
            joystickKnob.style.transform = 'translate(-50%, -50%)';
            moveState.forward = false; moveState.backward = false; moveState.left = false; moveState.right = false; moveState.velocity = 0;
        }
    });
}

function updateJoystickMovement() {
    const threshold = 15; const angle = virtualJoystick.angle; const cos = Math.cos(angle); const sin = Math.sin(angle);
...
function updateVirtualJoystickVisual() { const knob = document.getElementById('joystick-knob'); if (knob) knob.style.transform = 'translate(-50%, -50%)'; }

let isTeleporting = false; let teleportTarget = null; let teleportProgress = 0;

function setupTeleport() {
    let mouseDownTime = 0; let isMouseDragging = false;
...
function performTeleport() {
    ...
    if (floorIntersects.length > 0) {
        ...
        teleportTarget = targetPoint.clone(); teleportTarget.y = 1.6;
        isTeleporting = true; teleportProgress = 0;
...
function updateTeleport() {
    ...
    if (teleportProgress >= 1) { camera.position.copy(teleportTarget); isTeleporting = false; teleportTarget = null; teleportProgress = 0; showNotification('Chegou!', 'success'); }
    else { const lerpPos = camera.position.clone().lerp(teleportTarget, 0.15); camera.position.copy(lerpPos); }
}
...
const floorMesh = new THREE.Mesh(floorGeometry, floorMaterial); floorMesh.rotation.x = -Math.PI / 2; floorMesh.receiveShadow = true; scene.add(floorMesh);

const ceilingGeometry = new THREE.PlaneGeometry(20, 20);
const ceilingMaterial = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.9, metalness: 0.05 });
const ceilingMesh = new THREE.Mesh(ceilingGeometry, ceilingMaterial); ceilingMesh.rotation.x = Math.PI / 2; ceilingMesh.position.y = 4; ceilingMesh.receiveShadow = true; scene.add(ceilingMesh);

const wallGeometry = new THREE.PlaneGeometry(10, 4);
const wallMesh = new THREE.Mesh(wallGeometry, wallMaterial); wallMesh.position.set(0, 2, -5); wallMesh.receiveShadow = true; scene.add(wallMesh);

const panelGeometry = new THREE.BoxGeometry(3, 2, 0.2);
const panelMesh = new THREE.Mesh(panelGeometry, panelMaterial); panelMesh.position.set(0, 1.5, -4.8); panelMesh.castShadow = true; panelMesh.receiveShadow = true; scene.add(panelMesh);

const demoFloorGeometry = new THREE.PlaneGeometry(4, 4);
const demoFloorMesh = new THREE.Mesh(demoFloorGeometry, demoFloorMaterial); demoFloorMesh.rotation.x = -Math.PI / 2; demoFloorMesh.position.set(0, 0.01, -3); demoFloorMesh.receiveShadow = true; scene.add(demoFloorMesh);
...
let currentTourIndex = 0; let isTouring = false; let tourInterval = null;
...
function moveToTourPoint(index) {
    ...
    const targetPos = new THREE.Vector3(...point.position); const targetLook = new THREE.Vector3(...point.lookAt);
    const startPos = camera.position.clone(); const startLook = controls.target.clone();
    let progress = 0;
    const animateTour = () => {
        if (!isTouring) return;
        progress += 0.02;
        if (progress >= 1) { camera.position.copy(targetPos); controls.target.copy(targetLook); controls.update(); return; }
        camera.position.lerpVectors(startPos, targetPos, progress); controls.target.lerpVectors(startLook, targetLook, progress); controls.update();
...
function createHUD() {
    const instructions = document.createElement('div'); instructions.id = 'instructions';
    ...
    const surfaceInfo = document.createElement('div'); surfaceInfo.id = 'surface-info';
    ...
    surfaceInfo.textContent = 'Superficie: Nenhuma'; document.body.appendChild(surfaceInfo);
    const captureBtn = document.createElement('button'); captureBtn.id = 'capture-btn';
    ...
    captureBtn.textContent = 'Capturar Mouse'; document.body.appendChild(captureBtn);
...
document.addEventListener('pointerlockchange', () => {
    ...
    if (captureBtn) { captureBtn.textContent = isLocked ? 'Mouse Capturado' : 'Capturar Mouse'; captureBtn.style.background = isLocked ? 'rgba(39,174,96,0.9)' : 'rgba(52,152,219,0.9)'; }
});

window.addEventListener('resize', () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); });

createVirtualJoystick(); createHUD(); setupTeleport();

function animate() {
    requestAnimationFrame(animate);
    updateMovement(); updateTeleport(); controls.update();
